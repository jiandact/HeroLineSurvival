Shader "Unlit/fire"
{
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			/*
			//SCALE
			float2 scale(float2 _scale) {
				float2 e = float2(_scale.x, 0.0, 0.0, _scale.y);
				return e;
			}
			*/
...
#define OCTAVES 4
			...
			float3 hsb2rgb(in float3 c) {
				float3 rgb = clamp(abs((c.x*6.0 + float3(0.0, 4.0, 2.0)-6.0*floor(c.x*6.0 + float3(0.0, 4.0, 2.0)/6.0)) - 3.0) - 1.0,0.0,1.0);
